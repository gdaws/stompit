...
OutgoingFrame.prototype.end = function(chunk, encoding, callback){
  ...
  if(chunk !== undefined && chunk !== null){
    this.write(chunk, encoding, function(error){
      ...
      self._endFrame(callback);
    });
  }
  else{
    this._endFrame(callback);
  }
};

OutgoingFrame.prototype._endFrame = function(callback){
...
function errorOnWrite(chunk, encoding, callback){
  callback(new Error('cannot write on closed stream'));
}
