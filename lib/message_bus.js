...
MessageBusConnection.prototype.destroy = function(exception){
    this.unsubscribeAll();
    StompServerConnection.prototype.destroy.apply(this, [exception]);
}
...
Subscription.prototype._findMessageIndexById = function(id){
    ...
    for(var i = 0; i < numPendingMessages; i++){
        if(pendingMessages[i] && pendingMessages[i].id === id){
            return i;
        }
    }
    return -1;
...
Subscription.prototype.sendMessage = function(frame, callback){
    ...
    frame.on("error", function(){
        ...
        // The consumer is now in a state where it can't read the next 
        // frame: it has to be disconnected without warning.
