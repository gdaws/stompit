...
function Client(transportSocket, options){
    ...
    this.on("end", function(){
        if(!self.isOutputEnded()){
            self.destroy(new Error("connection lost"));
        }
    });
...
Client.prototype.disconnect = function(callback){
    ...
    var endOutput = this._end.bind(this);
    var gotReceipt = false;
    
    var self = this;
    
    this.once("end", function(){
        if(!gotReceipt){
            self.destroy(new Error("connection lost while waiting for disconnect receipt"));
        }
    });
