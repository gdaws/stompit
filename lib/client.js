 ...
 * 
 * @constructor
 * @param {stream.Duplex} transportSocket TCP socket or another type of
 *  Stream.Duplex object that's ready to read and write. It's acceptable to 
 *  pass a socket object that's in connecting state.
 * @param {Object=} options
 * @param {Array} [options.heartbeat=[0,0]] Set the preferred heart beat 
 *  timings, in milliseconds. The first element is the send frequency. The
 *  class will try to ensure the send frequency is reached by sending frame 
 *  trailer bytes if the application is not creating busy enough traffic. The 
 *  second element is the expected receive frequency; i.e. the client expects 
 *  to see an increase in the number of bytes received on the socket every n 
 *  milliseconds). To understand how the client and server negotiate timings 
 *  read {@linkcode http://stomp.github.io/stomp-specification-1.1.html#Heart-beating STOMP 1.1 spec on Heart-beating}
 * @param {Number} [options.heartbeatDelayMargin=100] Milliseconds added to the
 *  the heart-beat receive frequency to account for network delay.
 */
 ...
 * @param {Object} [options]
 * @param options.onReceipt
 * @returns {OutgoingFrame}
 */
 ...
 * 
 * @param {Object} headers headers sent in the CONNECT frame
 * @param {String} [headers.host]
 * @param {String} [headers.login]
 * @param {String} [headers.passcode]
 * @param {String} [headers.heart-beat] This will overwrite the heartbeat value
 *  given to the constructor.
 * @param {Client~connectCallback} callback called on operation completion
 */
 ...
 * 
 * @param {Object} headers
 * @param {Object} [options] See {@link Client#sendFrame} options
 * @returns {OutgoingFrame}
 */
...
/**
 * Start a transaction with the server.
 * 
 * @param {Object} [headers]
 * @param {String} [headers.transaction] Set the transaction identifier. The
 *  method generates this identifier if this property is undefined.
 * @returns {Transaction}
 */
 ...
 * 
 * @param {Object} headers
 * @param {String} headers.destination A string value required by the server.
 * @param {String} [headers.ack="auto"] Set the message acknowledgment mode. 
 * @param {String} [headers.id] Set the subscription identifier. This method
 *  generates an identifier if this property is undefined.
 * @param {Client~onMessageCallback} onMessageCallback called each time a new
 *  message is received.
 ...
 * 
 * @param callback called on successful disconnect or called on error.
 */
