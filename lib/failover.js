...
Failover.prototype._createConnector = function(arg){
    ...
    var config = this._defaultConnectOptions;
    ...
    switch(typeof arg){
        ...
        case 'string':
            config = util.extend(config, this._parseServerUri(arg));
            break;
        
        case 'object':
            config = util.extend(config, arg);
            break;
...
Failover.prototype.connect = function(callback){
    ...
    var error = function(description){
        var connectError = new Error('could not connect' + (description ? ' (' + description + ')' : '') + (lastError && lastError.message ? ' last connect error: ' + lastError.message : ''));
        if(lastError){
